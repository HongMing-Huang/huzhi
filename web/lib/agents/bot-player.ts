...
export async function botReply(room: Room, bot: Player, turn: number): Promise<ChatMessage> {
  ...
  // 真人也会"发图不回话"：约 14% 概率 bot 直接用刘看山贴纸回复（对称真实，双方都能发表情包）
  if (secureRand() < 0.14) {
    const stickers: ChatMessage["sticker"][] = ["wave", "idle", "stroll"];
    return {
      id: `m_${Date.now()}_${secureRand().toString(36).slice(2, 8)}`,
      from: bot.id,
      text: "",
      ts: Date.now(),
      responseMs: wait,
      sticker: stickers[Math.floor(secureRand() * stickers.length)],
    };
  }
  return {
    id: `m_${Date.now()}_${secureRand().toString(36).slice(2, 8)}`,
