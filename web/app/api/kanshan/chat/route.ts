...
/** 规则上下文由代码确定性注入，不靠 prompt 背（避免"一个大 prompt 管理一切"）；数值给全，防止模型算术幻觉。 */
const MANUAL = `《社区手册》计分表：
- 信息流猜身份：识破 AI（生成内容）＝ +30 分；确认真人 ＝ +10 分；误判 ＝ −20 分。
- 识破伪装者：成功识破 ＝ 30 × 1.6 ＝ 48 分；被骗过（判为相反）按误判 −20 分。
- 1v1 对局：识破伪装者 +80；识破本色 AI +30；猜中真人 +10；伪装者整场未被识破 +50；下注赢家通吃。
- 名单不公开，无法从任何页面推断他人身份；内容不得自曝身份。回答计分只能引用上表数值。`;

/** 无凭证时的本地演示回复：按意图给诚实、有用的回答（不假装真 AI）。 */
function demoReply(q: string): string {
  const s = q.toLowerCase();
  if (/积分|得分|怎么算|奖励/.test(s))
    return "识破 AI 得 30 分，认出真人得 10 分，误判扣 20 分；识破伪装者 48 分（30×1.6）；对局里识破伪装者 80 分。规则我都记得，但今天我的模型没接上，以上是管理员手册的原文。";
  if (/怎么玩|玩法|规则/.test(s))
