...
- [x] **社区 / 玩法侧栏几何统一（2026-09-14）**：互动页外层由 1120px 居中壳改为与首页一致的 1432px 全局壳、40px 桌面边距和 24/48px 响应式列距；内容浏览与互动玩法互相切换时，247px 侧栏保持同一水平位置，不再左右跳动。

- [x] **居民频道侧栏连续性（2026-09-14）**：频道广场 `/channels` 与频道详情 `/channels/[id]` 接入共享 `SidebarPage`，进入频道和继续浏览具体频道时均保留侧栏，并持续高亮“居民频道”。
